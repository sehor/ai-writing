...
Done in P2-08: runtime events are structured single-line JSON on stdout via `app.observability`. Every record carries `request_id` (UUID assigned by the HTTP middleware, echoed as `X-Request-ID`, bound through a contextvar), plus `operation`, `project_id`, `aggregate_id`, `provider`, `duration_ms`, `result` and `error_code` where applicable; extra fields ride explicitly and empty fields are omitted. Instrumented boundaries: HTTP access (`http_request` with route template + status), outbox job execution (`outbox_job` with attempt count), and provider generation calls (`provider_call` at the service call sites — deliberately not by wrapping providers, so the registry keeps returning concrete instances). API keys and prose content are never logged.
...
Done in P2-09: the browser E2E gate became a required merge-blocking CI job, backup / restore controls reached the frontend (sidebar export downloads the ZIP package; import runs preview-first and requires an explicit overwrite confirmation when the target project already exists), and the happy-path E2E now also covers Step 8 parse into scene proposals with batch acceptance plus accepted-scene edit and revision restore versioning.

The next implementation focus is broadening E2E coverage to provider-backed paths and edge-case failure modes beyond the two suites.
