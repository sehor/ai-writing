      ...
      <section class="backup-panel" aria-label="Backup and restore">
        ...
        <div v-if="backupPreview" class="backup-preview">
          ...
          <div class="backup-preview-actions">
            ...
            <button type="button" class="secondary" :disabled="isImporting" @click="backups.discardPreview()">
              取消
            </button>
          </div>
        </div>
        ...
        <p v-if="exportError || importError" class="backup-error">{{ exportError || importError }}</p>
      </section>
    </aside>
</template>
